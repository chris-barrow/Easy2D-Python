...
def proc(fid2,NNODE,NELEM,X,Y,NODE,KIND,TEMP,XIPMAP,CA,CB,CC,FREC,Field,Exterior,Px,Py,VINF):
    if Exterior==3:
    ...
    # form system of eqs
    CP,A,B,QN = form(NNODE, NELEM, NODE, KIND, X, Y, TEMP, XIPMAP, CA, CB, CC,Exterior,PhiI)
    # solve
    XS = solve(A,B)
    # save solution
    ...
    if Field==2:
        PhiP,dPhidPX,dPhidPY,QN = field(fid2,Px,Py,FREC,NNODE,NELEM,KIND,NODE,X,Y,TEMP,DTDN,Exterior,VINF,PhiI)
    else:
        PhiP = []
        dPhidPX = []
        dPhidPY = []
