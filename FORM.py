...
def form(NNODE, NELEM, NODE, KIND, X, Y, TEMP, XIPMAP, CA, CB, CC, Exterior,
         PhiI):
    ...
    for IP in range(NNODE):
        ...
        for K in range(NELEM):
            ...
            for J in range(NL):
                ...
                YQ[J] = Y[int(IQ)]
            if ISING == 0:
                CP[IP], G, H, QN = elemt(XP, YP, NL, KINDI, XQ, YQ, CP[IP],
                                         Exterior)
            else:
                CP[IP], G, H, QN = sing(XP, YP, NL, KINDI, XQ, YQ, ISING,
                                        XIPMAP, CP[IP], Exterior)
